...
- **Une seule ligne de log par requête HTTP** (`event: 'http_request'`, `msg: 'request'`) et **par message WebSocket** (`event: 'ws_message'`, `msg: 'message'`), émise par un interceptor global (`Http` / `WsWideEventInterceptor`, `src/common/interceptors/`), succès comme erreur. Niveau dérivé du `statusCode` : `info` (< 400 ou absent), `warn` (4xx), `error` (5xx).
- Le contexte vit dans un `AsyncLocalStorage` (`nestjs-cls`), une entrée par requête / message. Pour y ajouter un champ : injecter `WideEventService` et appeler `enrich({ … })` (depuis un guard, interceptor, gateway ou service) ; depuis un filtre global (non injectable) `enrichWideEventFromCls({ … })`. **Ne jamais** ajouter de `logger.log(...)` ad hoc pour tracer une requête ou un message — enrichir le wide event.
- **Erreurs : jamais de ligne à part.** Les `ExceptionFilter` (HTTP + WS) et `WsErrorInterceptor` replient l'erreur dans le wide event via `enrichWideEventFromCls(toWideEventErrorFields(payload, exception))` → `errorCode` / `errorMessage` (+ `errorStack` pour les 5xx et `UNKNOWN_ERROR`). `logWsApiError` ne subsiste qu'en fallback hors contexte CLS (erreur levée par un guard, cycle connexion du gateway).
- Enrichissements déjà branchés : `AuthGuard` / `OptionalAuthGuard` → `userId` / `isAuthenticated` ; `RateLimitGuard` → `rateLimitBucket` / `rateLimitRemaining` ; `SessionGateway` → `sessionId` / `playerId`.
- `pino-http` `autoLogging` est à `false` (`common/logger/logger.params.ts`) : aucune ligne automatique.
