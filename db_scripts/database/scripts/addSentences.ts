        ...
        try {
            ...
            if (error.code === 11000) {
                ...
                const duplicateSentences = sentences
                    .filter((sentence) => error.writeErrors.some(
                        (writeError: any) => writeError.err.op.sentence === sentence.sentence
                    ))
                    .map((s) => s.sentence);

                console.log("Les sentences suivantes n'ont pas pu être ajoutées en raison de doublons :", duplicateSentences);
