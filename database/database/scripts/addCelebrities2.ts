...
import { celebrityCollection, GuessObject } from "../models/celebrity2";
...
export async function addCelebrities(celebrities: GuessObject[]) {
    try {
        ...
        try {
            ...
            console.log("Opérations en lot terminées avec succès :", result.insertedCount);
            return {
            ...
            if (error.code === 11000) {
                ...
                const duplicateNames = celebrities
                    .filter((celebrity) => error.writeErrors.some(
                        (writeError: any) => writeError.err.op.name === celebrity.name
                    ))
                    .map((c) => c.name);

                console.log("Les célébrités suivantes n'ont pas pu être ajoutées en raison de doublons :", duplicateNames);
