...
export async function searchGuessObjectByName(query: string): Promise<GuessObjectCandidate[]> {
    const response = await apiFetch(`${process.env.BACKEND_URL}/guess-objects/search?q=${encodeURIComponent(query)}`, {
        method: 'GET',
        headers: {
...
export async function searchGuessObjectById(id: string): Promise<GuessObjectCandidate> {
    ...
    const response = await apiFetch(`${process.env.BACKEND_URL}/guess-objects/search?id=${id}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
        },
...
export async function searchWorldLocationByName(query: string): Promise<WorldLocation[]> {
    const response = await apiFetch(`${process.env.BACKEND_URL}/world-location/search?q=${encodeURIComponent(query)}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
        },
...
export async function searchWorldLocationById(id: string, osm_type: string): Promise<WorldLocation> {
    ...
    const response = await apiFetch(`${process.env.BACKEND_URL}/world-location/search?id=${id}&osm_type=${osm_type}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
        },
